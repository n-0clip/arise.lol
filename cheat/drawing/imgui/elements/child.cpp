...
void c_gui::begin_child(std::string_view name, int x, int y, const ImVec2& size)
{
    ...
    if (child_size.x <= 0.f)
        child_size.x = (window->Size.x - elements->content.padding.x * 2.f - elements->content.spacing.x * (x - 1)) / x;
    if (child_size.y <= 0.f)
        child_size.y = (window->SizeFull.y - elements->content.padding.y * 2.f - elements->content.spacing.y * (y - 1)) / y;

    const ImVec2 pos = window->DC.CursorPos + ImVec2(0, var->window.titlebar);
    const ImRect bb(pos, pos + child_size);
    const float rounding = 10.f;

    ImDrawList* dl = window->DrawList;

    draw->fade_rect_filled(dl, bb.Min, bb.Max,
        draw->get_clr(clr->window.background_one),
        draw->get_clr(clr->window.background_two),
        fade_direction::vertically);

    dl->AddRect(bb.Min, bb.Max, draw->get_clr(clr->window.stroke), rounding, ImDrawFlags_None, 1.2f);
    ...
    begin_child_ex(name.data(), id, x, y, child_size,
